...
def parse_image(data_record, dim_image, N_labels):
    ...
    sequence_features = {
        'pT1'  : tf.io.FixedLenSequenceFeature([dim_image[0][0]], tf.float32),
        'pT2'  : tf.io.FixedLenSequenceFeature([dim_image[0][0]], tf.float32),
        'Qk1' : tf.io.FixedLenSequenceFeature([dim_image[0][0]], tf.float32),
        'Qk2' : tf.io.FixedLenSequenceFeature([dim_image[0][0]], tf.float32),
        'pT3'  : tf.io.FixedLenSequenceFeature([dim_image[0][0]], tf.float32),
        'pT4'  : tf.io.FixedLenSequenceFeature([dim_image[0][0]], tf.float32),
        'Qk3' : tf.io.FixedLenSequenceFeature([dim_image[0][0]], tf.float32),
        'Qk4' : tf.io.FixedLenSequenceFeature([dim_image[0][0]], tf.float32),
    }
    ...
    labels = tf.convert_to_tensor(context_data['labels'])
    pT1  = tf.ensure_shape(tf.convert_to_tensor(sequence_data['pT1']), list(dim_image[0]))
    pT2  = tf.ensure_shape(tf.convert_to_tensor(sequence_data['pT2']), list(dim_image[0]))
    Qk1 = tf.ensure_shape(tf.convert_to_tensor(sequence_data['Qk1']), list(dim_image[0]))
    Qk2 = tf.ensure_shape(tf.convert_to_tensor(sequence_data['Qk2']), list(dim_image[0]))
    pT3  = tf.ensure_shape(tf.convert_to_tensor(sequence_data['pT1']), list(dim_image[0]))
    pT4  = tf.ensure_shape(tf.convert_to_tensor(sequence_data['pT2']), list(dim_image[0]))
    Qk3 = tf.ensure_shape(tf.convert_to_tensor(sequence_data['Qk1']), list(dim_image[0]))
    Qk4 = tf.ensure_shape(tf.convert_to_tensor(sequence_data['Qk2']), list(dim_image[0]))

    image = tf.stack([pT1,Qk1, pT2, Qk2, pT3, Qk3, pT4, Qk4], axis=-1)
