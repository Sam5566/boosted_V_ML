...
#dim_image = [[5, 5], [[-0.8, 0.8], [-0.8, 0.8]]]
best_model_dir = '/home/samhuang/ML/best_model/'
#save_model_name = best_model_dir+'best_model_ternary_T-CNN_event_kappa0.23/'
save_model_name = best_model_dir+'best_model_ternary2_T-CNN_event_kappa0.23/'
...
for i_try in range(Ntry):
    ...
    dataset_tr, tr_total = get_dataset(data_tr, repeat=False, 
                                   ...
                                   shuffle=shuffle_size_tr, N_labels=len(signal))
    ...
    #print(dataset_tr[0])
    # Create the model  
    model = models.CNN(dim_image=dim_image[0] + [4], n_class=len(signal))
    #print(model.summary())
