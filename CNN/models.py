...
class EFP1(tf.keras.Model):
    def __init__(self, name="EFP1"):
        super(EFP1, self).__init__(name=name)
    ...
    def call(self, inputs, training=False):
        latent_all = self.denses(inputs)    
        
        return self._output(latent_all)

class DNN1(tf.keras.Model):
    def __init__(self, name="DNN1"):
        super(DNN1, self).__init__(name=name)
...
class ImDNN1(tf.keras.Model):
    def __init__(self, name="ImDNN1", dim_image=(100, 100, 1)):
        super(ImDNN1, self).__init__(name=name)
...
class CNN1(tf.keras.Model):
    def __init__(self, name="CNN1", dim_image=(100, 100, 1)):
        super(CNN1, self).__init__(name=name)
...
class DCNN1(tf.keras.Model):
    def __init__(self, name="DCNN1", dim_image=(100, 100, 1)):
        super(DCNN1, self).__init__(name=name)
        
        """h2ptjl Channel"""
        self.h2ptjl = tf.keras.Sequential([
            tf.keras.layers.Lambda(lambda x: tf.expand_dims(x[:, :, :(dim_image[0])], -1)),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Conv2D(32, (3,3), padding='same', activation='relu'),
            tf.keras.layers.Conv2D(32, (3,3), padding='same', activation='relu'),
            tf.keras.layers.MaxPool2D((2,2)),
        ...
        self.denses = tf.keras.Sequential([
            tf.keras.layers.Lambda(lambda x: x[:, :15, -1]),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Dense(256, activation='relu'),
            tf.keras.layers.Dense(256, activation='relu'),
            tf.keras.layers.Dropout(0.5)
    ...
    def call(self, inputs, training=False):
        ...
        """dense"""
        latent_denses = self.denses(inputs)
        
        """Output"""
        latent_all = tf.concat([latent_h2ptjl, latent_denses], axis=1)
