...
#datanames = ['VBF_H5pp_ww_jjjj_UFO', 'VBF_H5mm_ww_jjjj_UFO', 'VBF_H5z_zz_jjjj_UFO']
...
if (figure_data):
    ...
    for dataname in datanames:
        ...
        df = pd.read_csv(datafolder+dataname+'_properties.txt', index_col=0).replace('W+/W+',1).replace('W-/W-',2).replace('Z/Z',3).replace('W-/W+',4).replace('W+/W-',4).replace('W+/Z',5).replace('Z/W+',5).replace('W-/Z',6).replace('Z/W-',6)[0:300000]
        print (df)
        dfs.append(df)
