...
def create_TFRecord(npy_files, Ndata, shuffled_dataName):
    datasizes = []

    NameOfDirectory = '_and_'.join([string.split('.npy')[0] for string in npy_files])
    print (type(npy_files))
    ...
    with tqdm(total=datasize) as pbar:
        with ExitStack() as stack:
            npy_readers = [stack.enter_context(open(shuffled_dataName, 'rb')) for npy_file in npy_files]
