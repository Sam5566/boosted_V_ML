...
def get_sequence_example_object(data_element_dict):
    ...
    feature_lists = tf.train.FeatureLists(
            feature_list=
            {
                ...
                'pT3'   : image_feature(data_element_dict['pT3']),
                'pT4'   : image_feature(data_element_dict['pT4']),
                'Qk3'   : image_feature(data_element_dict['Qk3']),
                'Qk4'   : image_feature(data_element_dict['Qk4']),
            }
    )
...
def determine_entry(entry, idx):
    ...
    if (input_entry=='Z/Z'):
        ...
        entry[0]=[0,1,0,0,0,0]
        #N_Wm += 1
    elif (input_entry=='W+/W-') or (input_entry=='W-/W+'):
        #print ("W-")
        entry[0]=[0,0,0,1,0,0]
        #N_Wm += 1
    elif (input_entry=='W+/Z') or (input_entry=='Z/W+'):
        #print ("W-")
        entry[0]=[0,0,0,0,1,0]
        #N_Wm += 1
    elif (input_entry=='Z/W-') or (input_entry=='W-/Z'):
        #print ("W-")
        entry[0]=[0,0,0,0,0,1]
        #N_Wm += 1
    elif (entry[0]==[0,1,0,0,0,0]) or (entry[0]==[1,0,0,0,0,0]) or (entry[0]==[0,0,1,0,0,0]) or (entry[0]==[0,0,0,1,0,0]) or (entry[0]==[0,0,0,0,1,0]) or (entry[0]==[0,0,0,0,0,1]):
        pass
    else:
...
def create_TFRecord(npy_files):
    ...
    datasizes = [min(datasizes[0],250000), min(datasizes[1],250000), min(datasizes[2],250000), min(datasizes[3],250000), min(datasizes[4],250000), min(datasizes[5],250000)]
        ...
        with ExitStack() as stack:
            ...
            with tf.io.TFRecordWriter(NameOfDirectory +'/train.tfrecord') as tfwriter:
                for idx in tr_list:
                    ...
                    dict_obj = {'labels': entry[0], 'pT1': entry[1], 'Qk1':entry[2], 'pT2': entry[3], 'Qk2': entry[4], 'pT3': entry[5], 'Qk3':entry[6], 'pT4': entry[7], 'Qk4': entry[8]}
