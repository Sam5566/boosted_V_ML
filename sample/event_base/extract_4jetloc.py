...
def sample_selection(File, histbins, histranges, kappa, signal_label, pbar, tfwriter, imagewriter, histbins_true):
	data_collect = []
	evt_total = 0
	json_list = []
	Npass = np.zeros(5)
	...
	for evt_id, evt in enumerate(File):
		...
		Npass[0] += 1
		particle_list = [evt.Particle[id_w1], evt.Particle[id_w2], evt.Particle[id_q1], evt.Particle[id_q2]]
		...
		for ii in range(len(jet_list)):
			...
			if (np.isnan(histbins_true)):
				...
				obs[-1][0], _, _ = np.histogram2d(eta_list, phi_list, range=histranges, bins=histbins_true, weights=pT_list)
				obs[-1][1], _, _ = np.histogram2d(eta_list, phi_list, range=histranges, bins=histbins_true, weights=Qk_list)
				obs[-1][0] = cv2.resize(obs[-1][0], histbins, interpolation = cv2.INTER_LINEAR)
				obs[-1][1] = cv2.resize(obs[-1][1], histbins, interpolation = cv2.INTER_LINEAR)
			...
			'''
				...
				if (particle_list[ii].PID>0) and (particle_list[ii].PID<6):
					#particle_type+='/q+'
				elif (particle_list[ii].PID<0) and (particle_list[ii].PID>-6):
					#particle_type+='/q-'
				else:
					print (particle_list[ii].PID)
					print ("no particle type")
		...
		#jiejiprint (particle_type, np.array(particle_type))
		data_collect.append([particle_type, obs_mass[0], obs_Qk[0], obs_mass[1], obs_Qk[1]])
		image = np.array([np.array(particle_type), obs[0][0], obs[0][1], obs[1][0], obs[1][1], obs[2][0], obs[2][1], obs[3][0], obs[3][1]], dtype=object)
			#print (image)
...
def main():
	...
	outputfiledir = sys.argv[2].split('/')[0]+'/'+ sys.argv[2].split('/')[1]+'/'+ sys.argv[2].split('/')[2]+'/'+ sys.argv[2].split('/')[3]+'/' + sys.argv[2].split('/')[4]+'/' + "event_base/samples_kappa"+str(kappa)+'_4jetloc/'
	...
	with tqdm(total=chain.GetEntries()) as pbar:
		with tf.io.TFRecordWriter(outname) as tfwriter:
			with open(imagename, 'wb') as imagewriter:
				evt_total = 0
				evt_total, data_collection, Npass = sample_selection(chain, histbins, histranges, kappa, signal_label, pbar, tfwriter, imagewriter, histbins_true)
				#for id, evt in enumerate(chain):z
