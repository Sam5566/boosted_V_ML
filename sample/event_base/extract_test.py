...
def preprocess(jet_list, constituents_list, kappa):
...
def sample_selection(File, histbins, histranges, kappa, signal_label, pbar, tfwriter, imagewriter):
	...
	for evt_id, evt in enumerate(File):
		...
		pt_news, eta_news, phi_news, Q_kappas, E_news = preprocess(jet_list, constituents_list, kappa)
		id_consti = 0
		for jet_id, jet in enumerate(jet_list):
			for id_1st, consti in enumerate(constituents_list[jet_id]):
				...
				pt_news[id_consti]/jet.PT, Rin, Q_kappas[id_consti], E_news[id_consti]])
				id_consti += 1
		...
		hE, _, _ = np.histogram2d(eta_list, phi_list, range=histranges, bins=histbins, weights=E_list)
...
def main():
	histbins = [75, 75]
	histranges = [[-1.5, 1.5], [-np.pi, np.pi]]
	kappa = float(sys.argv[1])

	inname = sys.argv[2].split('/')[5] #// should be changed with different directory structure
	outputfiledir = sys.argv[2].split('/')[0]+'/'+ sys.argv[2].split('/')[1]+'/'+ sys.argv[2].split('/')[2]+'/'+ sys.argv[2].split('/')[3]+'/' + sys.argv[2].split('/')[4]+'/' + "event_base/samples_kappa"+str(kappa)+'_test/'
